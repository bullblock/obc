...
variable "admin_pass" {
  ...
  validation {
    condition = (
    (length(var.admin_pass) >= 8 && length(var.admin_pass) <= 26 &&
    length(regexall(".*[a-zA-Z].*", var.admin_pass)) > 0 &&
    length(regexall(".*[0-9].*", var.admin_pass)) > 0 &&
    #     length(regexall(".*[!@$%^-_=+[{}]:,./?].*", var.admin_password)) > 0
    length(regexall("[!@\\$%\\^\\-_=\\+\\[\\{\\}\\]:,\\./\\?]", var.admin_pass)) > 0
    ) || (length(var.admin_pass) == 0 || var.admin_pass == null))
...
variable "key_pair" {
  type        = string
  description = "The name of the SSH key pair used to log in to CES."
  default     = null
  nullable = true
  sensitive   = true
}

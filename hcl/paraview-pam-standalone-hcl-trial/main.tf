...
module "vpc" {
  ...
  vpc_name          = "paraview-pam-standalone-trial-vpc"
  ...
  subnet_name           = "paraview-pam-standalone-trial-subnet"
...
module "ecs-security-group" {
  ...
  secgroup_name           = "paraview-pam-standalone-trial-secgroup"
...
module "ecs" {
  ...
  instance_name = "paraview-pam-standalone-trial"
  ...
  /opt/scripts/config.sh $public_ip
  sleep 3m
...
module "eip" {
  ...
  eip_name    = "paraview-pam-standalone-trial-eip"

  publicip_type         = local.publicip_type
  # bandwidth_name        = format("%s-%s", local.app_id, "bandwidth")
  bandwidth_name        = "paraview-pam-standalone-trial-bandwidth"

...
module "vpc" {
  ...
  vpc_name          = "paraview-mfa-standalone-trial-vpc"
  ...
  subnet_name           = "paraview-mfa-standalone-trial-subnet"
...
module "ecs-security-group" {
  ...
  secgroup_name           = "paraview-mfa-standalone-trial-secgroup"
...
module "ecs" {
  ...
  instance_name = "paraview-mfa-standalone-trial"
  ...
  /home/latest/config.sh $public_ip
  sleep 5m
...
module "eip" {
  ...
  eip_name    = "paraview-mfa-standalone-trial-eip"

  publicip_type         = local.publicip_type
  # bandwidth_name        = format("%s-%s", local.app_id, "bandwidth")
  bandwidth_name        = "paraview-mfa-standalone-trial-bandwidth"
